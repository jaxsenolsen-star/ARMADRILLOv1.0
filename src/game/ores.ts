...
export function oreWeights(depth: number, list: Ore[] = ORES): number[] {
  return list.map((o) => {
...
export function pickOre(depth: number, rnd: number, list: Ore[] = ORES): Ore {
  const w = oreWeights(depth, list);
