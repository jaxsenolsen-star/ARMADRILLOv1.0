import { pickOre, type Ore } from "./ores";
import { getShell, type ShellEffects } from "./shells";
import type { SaveData } from "./storage";
import type { Planet } from "./planets";

export const TS = 22;
const CHUNK = 16;

/** Pixels of travel within STOP_TIME that still counts as "completely stopped". */
const STOP_DRIFT = 3.5;
/** Seconds the dillo must stay put before the run ends. */
const STOP_TIME = 0.9;

const EMPTY = 0;
const DIRT = 1;
const ROCK = 2;
const LAVA = 3;
const GRASS = 4;
const BEDROCK = 5;
const ORE_BASE = 100;

export type RunResult = {
  depth: number;
  value: number;
  ores: Record<string, number>;
  planet: string;
};

/* ---------------------------------- rng ---------------------------------- */

function hash2(x: number, y: number, s: number) {
  let h = x * 374761393 + y * 668265263 + s * 1442695040888963;
  h = (h ^ (h >>> 13)) * 1274126177;
  return ((h ^ (h >>> 16)) >>> 0) / 4294967296;
}
function smooth(t: number) {
  return t * t * (3 - 2 * t);
}
function valueNoise(x: number, y: number, s: number) {
  const x0 = Math.floor(x);
  const y0 = Math.floor(y);
  const fx = smooth(x - x0);
  const fy = smooth(y - y0);
  const a = hash2(x0, y0, s);
  const b = hash2(x0 + 1, y0, s);
  const c = hash2(x0, y0 + 1, s);
  const d = hash2(x0 + 1, y0 + 1, s);
  return (a * (1 - fx) + b * fx) * (1 - fy) + (c * (1 - fx) + d * fx) * fy;
}

/* ------------------------------ tile sprites ------------------------------ */

const spriteCache = new Map<string, HTMLCanvasElement>();

function make(w: number, h: number) {
  const c = document.createElement("canvas");
  c.width = w;
  c.height = h;
  return c;
}

const PATCH = 8;

function dirtPatch(band: number, planet: Planet) {
  const key = `dp:${planet.id}:${band}`;
  const hit = spriteCache.get(key);
  if (hit) return hit;
  const size = PATCH * TS;
  const c = make(size, size);
  const g = c.getContext("2d")!;
  const p = planet.palette.bands[Math.min(band, planet.palette.bands.length - 1)]!;
  g.fillStyle = p.base;
  g.fillRect(0, 0, size, size);
  for (let i = 0; i < 90; i++) {
    const x = hash2(i, band, 101) * size;
    const y = hash2(i + 200, band, 103) * size;
    const w = 6 + hash2(i + 300, band, 107) * 22;
    const h = 4 + hash2(i + 400, band, 109) * 14;
    g.globalAlpha = 0.16;
    g.fillStyle = hash2(i, band, 111) > 0.5 ? p.light : p.dark;
    g.fillRect(Math.floor(x), Math.floor(y), Math.floor(w), Math.floor(h));
  }
  for (let i = 0; i < 900; i++) {
    const x = Math.floor(hash2(i, band, 113) * size);
    const y = Math.floor(hash2(i + 500, band, 127) * size);
    const r = hash2(i + 900, band, 131);
    g.globalAlpha = 0.35 + r * 0.45;
    g.fillStyle = r > 0.55 ? p.light : p.dark;
    const sz = r > 0.93 ? 3 : r > 0.7 ? 2 : 1;
    g.fillRect(x, y, sz, sz);
    if (r > 0.96) {
      g.fillStyle = "rgba(0,0,0,0.35)";
      g.fillRect(x, y + sz, sz, 1);
    }
  }
  g.globalAlpha = 1;
  spriteCache.set(key, c);
  return c;
}

function rockPatch(band: number, planet: Planet) {
  const key = `rp:${planet.id}:${band}`;
  const hit = spriteCache.get(key);
  if (hit) return hit;
  const size = PATCH * TS;
  const c = make(size, size);
  const g = c.getContext("2d")!;
  const [s1, s2, s3] = planet.palette.rockShades;
  g.fillStyle = planet.palette.rock;
  g.fillRect(0, 0, size, size);
  for (let i = 0; i < 120; i++) {
    const x = Math.floor(hash2(i, band, 211) * size);
    const y = Math.floor(hash2(i + 300, band, 213) * size);
    const w = 8 + Math.floor(hash2(i + 600, band, 217) * 22);
    const h = 6 + Math.floor(hash2(i + 900, band, 219) * 16);
    const r = hash2(i, band, 221);
    g.fillStyle = r > 0.66 ? s1! : r > 0.33 ? s2! : s3!;
    g.fillRect(x, y, w, h);
    g.fillStyle = "rgba(255,255,255,0.16)";
    g.fillRect(x, y, w, 2);
    g.fillStyle = "rgba(0,0,0,0.22)";
    g.fillRect(x, y + h - 2, w, 2);
  }
  for (let i = 0; i < 700; i++) {
    const x = Math.floor(hash2(i, band, 223) * size);
    const y = Math.floor(hash2(i + 400, band, 227) * size);
    const r = hash2(i + 800, band, 229);
    g.globalAlpha = 0.4 + r * 0.4;
    g.fillStyle = r > 0.5 ? s1! : s3!;
    g.fillRect(x, y, r > 0.94 ? 2 : 1, r > 0.94 ? 2 : 1);
  }
  g.globalAlpha = 1;
  spriteCache.set(key, c);
  return c;
}

function grassSprite(v: number, planet: Planet) {
  const key = `g:${planet.id}:${v}`;
  const hit = spriteCache.get(key);
  if (hit) return hit;
  const c = make(TS, TS);
  const g = c.getContext("2d")!;
  const base = planet.palette.grass;
  // Darken the grass base for layered effect
  const dark = shade(base, -0.35);
  const mid = shade(base, -0.12);
  const light = shade(base, 0.2);
  g.fillStyle = dark;
  g.fillRect(0, 0, TS, 9);
  g.fillStyle = mid;
  g.fillRect(0, 0, TS, 6);
  g.fillStyle = light;
  g.fillRect(0, 0, TS, 3);
  for (let x = 0; x < TS; x++) {
    const h = Math.floor(hash2(x, v, 311) * 4);
    g.fillStyle = hash2(x, v, 313) > 0.5 ? shade(base, 0.3) : mid;
    g.fillRect(x, -h + 1, 1, h + 2);
  }
  g.fillStyle = "rgba(0,0,0,0.28)";
  g.fillRect(0, 9, TS, 2);
  spriteCache.set(key, c);
  return c;
}

/** Lighten (positive) or darken (negative) a hex color by amt (0..1). */
function shade(hex: string, amt: number): string {
  const n = parseInt(hex.slice(1), 16);
  let r = (n >> 16) & 0xff;
  let g = (n >> 8) & 0xff;
  let b = n & 0xff;
  if (amt >= 0) {
    r = Math.min(255, Math.round(r + (255 - r) * amt));
    g = Math.min(255, Math.round(g + (255 - g) * amt));
    b = Math.min(255, Math.round(b + (255 - b) * amt));
  } else {
    const d = 1 + amt;
    r = Math.max(0, Math.round(r * d));
    g = Math.max(0, Math.round(g * d));
    b = Math.max(0, Math.round(b * d));
  }
  return `#${((r << 16) | (g << 8) | b).toString(16).padStart(6, "0")}`;
}

function gem(
  g: CanvasRenderingContext2D,
  x: number,
  y: number,
  s: number,
  o: Ore,
  rot: number,
) {
  const [dark, base, light, spark] = o.colors;
  g.save();
  g.translate(x, y);
  g.rotate(rot);
  g.fillStyle = "rgba(0,0,0,0.4)";
  g.fillRect(-s / 2 + 1.5, -s / 2 + 1.5, s, s);
  g.fillStyle = base;
  g.fillRect(-s / 2, -s / 2, s, s);
  g.fillStyle = dark;
  g.beginPath();
  g.moveTo(-s / 2, s / 2);
  g.lineTo(s / 2, s / 2);
  g.lineTo(s / 2, -s / 2);
  g.closePath();
  g.fill();
  g.fillStyle = light;
  g.fillRect(-s / 2 + 1, -s / 2 + 1, s * 0.48, s * 0.48);
  g.fillStyle = spark;
  g.fillRect(-s / 2 + 1.5, -s / 2 + 1.5, Math.max(1, s * 0.22), Math.max(1, s * 0.22));
  g.strokeStyle = "rgba(0,0,0,0.5)";
  g.lineWidth = 1;
  g.strokeRect(-s / 2 + 0.5, -s / 2 + 0.5, s - 1, s - 1);
  g.restore();
}

function oreSprite(oreIdx: number, v: number, planet: Planet) {
  const key = `o:${planet.id}:${oreIdx}_${v}`;
  const hit = spriteCache.get(key);
  if (hit) return hit;
  const o = planet.ores[oreIdx]!;
  const c = make(TS, TS);
  const g = c.getContext("2d")!;
  if (o.hardness >= 4) {
    const rg = g.createRadialGradient(TS / 2, TS / 2, 1, TS / 2, TS / 2, TS * 0.7);
    rg.addColorStop(0, o.colors[2] + "55");
    rg.addColorStop(1, "rgba(0,0,0,0)");
    g.fillStyle = rg;
    g.fillRect(0, 0, TS, TS);
  }
  const n = 3 + Math.floor(hash2(v, oreIdx, 21) * 3);
  for (let i = 0; i < n; i++) {
    const x = 5 + hash2(i, v + oreIdx, 31) * (TS - 10);
    const y = 5 + hash2(i + 5, v * 2 + oreIdx, 41) * (TS - 10);
    const sz = 5 + hash2(i + 11, v + oreIdx, 51) * 6;
    gem(g, x, y, sz, o, (hash2(i + 3, v, 61) - 0.5) * 0.9);
  }
  spriteCache.set(key, c);
  return c;
}

/* --------------------------------- chunks --------------------------------- */

type Chunk = Int16Array;

class World {
  chunks = new Map<string, Chunk>();
  seed = Math.floor(Math.random() * 1e6);
  doubleOres = false;
  planet: Planet;

  constructor(planet: Planet) {
    this.planet = planet;
  }

  key(cx: number, cy: number) {
    return cx + "," + cy;
  }

  chunk(cx: number, cy: number): Chunk {
    const k = this.key(cx, cy);
    let c = this.chunks.get(k);
    if (c) return c;
    c = new Int16Array(CHUNK * CHUNK);
    this.gen(c, cx, cy);
    this.chunks.set(k, c);
    return c;
  }

  gen(c: Chunk, cx: number, cy: number) {
    const s = this.seed;
    for (let j = 0; j < CHUNK; j++) {
      const ty = cy * CHUNK + j;
      for (let i = 0; i < CHUNK; i++) {
        const tx = cx * CHUNK + i;
        let t: number = DIRT;
        if (ty < 0) t = EMPTY;
        else if (ty === 0) t = GRASS;
        else {
          const depth = ty;
          const cave = valueNoise(tx * 0.13, ty * 0.11, s);
          if (cave > 0.74 && depth > 6) t = EMPTY;
          const rockN = valueNoise(tx * 0.09 + 40, ty * 0.08 + 90, s + 3);
          const rockChance = 0.76 - Math.min(0.14, depth / 40000);
          if (t === DIRT && rockN > rockChance && depth > 26) t = ROCK;
          const lavaN = valueNoise(tx * 0.07 - 60, ty * 0.06 - 20, s + 7);
          const lavaThresh = 0.86 - Math.min(0.16, depth / 12000);
          if (depth > 45 && lavaN > lavaThresh) t = LAVA;
        }
        c[j * CHUNK + i] = t;
      }
    }
    const attempts = this.doubleOres ? 14 : 7;
    for (let a = 0; a < attempts; a++) {
      const r = hash2(cx * 91 + a, cy * 57 + a * 13, s + 17);
      if (r > 0.88) continue;
      const ox = Math.floor(hash2(a, cx + cy * 7, s + 23) * CHUNK);
      const oy = Math.floor(hash2(a + 4, cx * 3 + cy, s + 29) * CHUNK);
      const ty = cy * CHUNK + oy;
      if (ty < 4) continue;
      const ore = pickOre(ty, hash2(a + 8, cx * 5 + cy * 11, s + 31), this.planet.ores);
      const idx = this.planet.ores.indexOf(ore);
      const size = 6 + Math.floor(hash2(a + 12, cx + cy, s + 37) * 9);
      let px = ox;
      let py = oy;
      for (let b = 0; b < size; b++) {
        if (px >= 0 && px < CHUNK && py >= 0 && py < CHUNK) {
          const cur = c[py * CHUNK + px];
          if (cur === DIRT || cur === ROCK) c[py * CHUNK + px] = ORE_BASE + idx;
        }
        const dir = hash2(a * 3 + b, cx * 13 + cy * 3 + b, s + 41);
        if (dir < 0.25) px++;
        else if (dir < 0.5) px--;
        else if (dir < 0.75) py++;
        else py--;
      }
    }
  }

  prune(cx: number, cy: number) {
    if (this.chunks.size < 900) return;
    for (const k of this.chunks.keys()) {
      const [a, b] = k.split(",").map(Number) as [number, number];
      if (Math.abs(a - cx) > 8 || Math.abs(b - cy) > 8) this.chunks.delete(k);
    }
  }

  get(tx: number, ty: number) {
    if (ty < 0) return EMPTY;
    const cx = Math.floor(tx / CHUNK);
    const cy = Math.floor(ty / CHUNK);
    const c = this.chunk(cx, cy);
    return c[(ty - cy * CHUNK) * CHUNK + (tx - cx * CHUNK)]!;
  }

  set(tx: number, ty: number, v: number) {
    if (ty < 0) return;
    const cx = Math.floor(tx / CHUNK);
    const cy = Math.floor(ty / CHUNK);
    const c = this.chunk(cx, cy);
    c[(ty - cy * CHUNK) * CHUNK + (tx - cx * CHUNK)] = v;
  }
}

/* --------------------------------- helpers -------------------------------- */

function tileHardness(t: number, planet: Planet) {
  if (t === DIRT || t === GRASS) return 0;
  if (t === ROCK) return 2;
  if (t >= ORE_BASE) return planet.ores[t - ORE_BASE]!.hardness;
  return 99;
}

type Particle = {
  x: number;
  y: number;
  vx: number;
  vy: number;
  life: number;
  max: number;
  c: string;
  s: number;
};
type Popup = { x: number; y: number; text: string; c: string; life: number };

type Ufo = {
  x: number;
  y: number;
  vx: number;
  hp: number;
  shootCd: number;
  hover: number;
};

type Projectile = {
  x: number;
  y: number;
  vx: number;
  vy: number;
  life: number;
  fromUfo: boolean;
};

export type GameHandle = { destroy: () => void };

export function createGame(
  canvas: HTMLCanvasElement,
  opts: {
    save: SaveData;
    planet: Planet;
    onEnd: (r: RunResult) => void;
    onQuit: () => void;
  },
): GameHandle {
  const ctx = canvas.getContext("2d")!;
  const planet = opts.planet;
  const lv = (id: string) => opts.save.levels[id] ?? 0;

  const shell = getShell(opts.save.equippedShell);
  const fx: ShellEffects = shell.effects;

  const maxHp = (100 + lv("plating") * 12) * (1 + (fx.hpBonus ?? 0));
  const powerMul = 1 + lv("launcher") * 0.05;
  const speedMul = (1 + lv("spurs") * 0.04) * (1 + (fx.speedBonus ?? 0));
  const fuelMax = 100 * (1 + lv("stamina") * 0.10) * (1 - (fx.fuelReduction ?? 0));
  const drill = lv("drill") + (fx.drillBonus ? 1 : 0) + (fx.drillTier ?? 0);
  const magnetR = fx.magnet ? 2.7 * TS * (fx.magnetPower ?? 1) : 0;
  const fortune = 1 + lv("fortune") * 0.07 + (fx.coinBonus ?? 0);
  const armorMul = 1 - (fx.armorBonus ?? 0);
  const lavaMul = 1 - (fx.lavaReduction ?? 0);

  const GRAV = planet.gravity;
  const FUEL_MUL = planet.fuelMul;

  const world = new World(planet);
  world.doubleOres = fx.doubleOres ?? false;

  const p = {
    x: 0,
    y: -30,
    vx: 0,
    vy: 0,
    r: 10,
    rot: 0,
    hp: maxHp,
    fuel: fuelMax,
  };

  let state: "aim" | "roll" | "over" = "aim";
  let aiming = false;
  let aimX = 0;
  let aimY = 0;
  let camX = p.x;
  let camY = 0;
  let shake = 0;
  let hurtFlash = 0;
  let burnCd = 0;
  let anchorX = p.x;
  let anchorY = p.y;
  let stopTime = 0;
  let dryFlash = 0;
  let wasDry = false;
  let value = 0;
  let maxDepth = 0;
  const collected: Record<string, number> = {};
  const particles: Particle[] = [];
  const popups: Popup[] = [];
  const ufos: Ufo[] = [];
  const projectiles: Projectile[] = [];
  let ufoSpawnCd = 8;
  const keys = new Set<string>();
  let touchDir = 0;
  let running = true;
  let W = 0;
  let H = 0;
  let t = 0;

  /* ------------------------------- resizing ------------------------------- */
  function resize() {
    const dpr = Math.min(2, window.devicePixelRatio || 1);
    W = canvas.clientWidth;
    H = canvas.clientHeight;
    canvas.width = Math.floor(W * dpr);
    canvas.height = Math.floor(H * dpr);
    ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    ctx.imageSmoothingEnabled = false;
  }
  resize();
  const ro = new ResizeObserver(resize);
  ro.observe(canvas);

  /* --------------------------------- input -------------------------------- */
  function pos(e: PointerEvent) {
    const r = canvas.getBoundingClientRect();
    return { x: e.clientX - r.left, y: e.clientY - r.top };
  }
  function onDown(e: PointerEvent) {
    canvas.setPointerCapture(e.pointerId);
    const q = pos(e);
    if (state === "aim") {
      aiming = true;
      aimX = q.x;
      aimY = q.y;
    } else if (state === "roll") {
      touchDir = q.x < W / 2 ? -1 : 1;
    }
  }
  function onMove(e: PointerEvent) {
    const q = pos(e);
    if (aiming) {
      aimX = q.x;
      aimY = q.y;
    } else if (state === "roll" && touchDir !== 0) {
      touchDir = q.x < W / 2 ? -1 : 1;
    }
  }
  function onUp() {
    if (aiming && state === "aim") launch();
    aiming = false;
    touchDir = 0;
  }
  function onKeyDown(e: KeyboardEvent) {
    keys.add(e.key.toLowerCase());
    if (e.key === "Escape") opts.onQuit();
    if ([" ", "ArrowLeft", "ArrowRight", "ArrowDown", "ArrowUp"].includes(e.key))
      e.preventDefault();
  }
  function onKeyUp(e: KeyboardEvent) {
    keys.delete(e.key.toLowerCase());
  }
  canvas.addEventListener("pointerdown", onDown);
  canvas.addEventListener("pointermove", onMove);
  window.addEventListener("pointerup", onUp);
  window.addEventListener("keydown", onKeyDown);
  window.addEventListener("keyup", onKeyUp);

  function launch() {
    const sx = W / 2;
    const sy = H * 0.32;
    let dx = sx - aimX;
    let dy = sy - aimY;
    const len = Math.hypot(dx, dy);
    if (len < 12) return;
    const power = Math.min(len, 170) / 170;
    dx /= len;
    dy /= len;
    const speed = (420 + power * 780) * powerMul;
    p.vx = dx * speed;
    p.vy = Math.max(dy * speed, -220);
    state = "roll";
    shake = 8;
    burst(p.x, p.y, 22, "#f4d58d");
  }

  /* ------------------------------- particles ------------------------------ */
  function burst(x: number, y: number, n: number, c: string, spd = 180) {
    for (let i = 0; i < n; i++) {
      const a = Math.random() * Math.PI * 2;
      const s = Math.random() * spd;
      particles.push({
        x,
        y,
        vx: Math.cos(a) * s,
        vy: Math.sin(a) * s,
        life: 0.4 + Math.random() * 0.5,
        max: 0.9,
        c,
        s: 2 + Math.random() * 3,
      });
    }
  }
  function popup(x: number, y: number, text: string, c: string) {
    popups.push({ x, y, text, c, life: 1.1 });
  }

  /* -------------------------------- digging ------------------------------- */
  function hitTile(tx: number, ty: number, t: number): boolean {
    const h = tileHardness(t, planet);
    if (h > drill) return false;
    const bandIdx = Math.min(
      Math.floor(ty / 110),
      planet.palette.bands.length - 1,
    );
    world.set(tx, ty, EMPTY);
    const cx = tx * TS + TS / 2;
    const cy = ty * TS + TS / 2;
    if (t >= ORE_BASE) {
      const o = planet.ores[t - ORE_BASE]!;
      const gain = Math.round(o.value * fortune);
      value += gain;
      collected[o.id] = (collected[o.id] ?? 0) + 1;
      popup(cx, cy, `+${gain} ${o.name}`, o.colors[2]);
      burst(cx, cy, 14, o.colors[2], 150);
      shake = Math.max(shake, 3);
    } else {
      const band = planet.palette.bands[bandIdx]!;
      burst(cx, cy, 6, band.light, 110);
      if (t === ROCK) {
        burst(cx, cy, 6, "#b9bec7", 130);
        shake = Math.max(shake, 2);
      }
    }
    return true;
  }

  function collide(dt: number) {
    const minTx = Math.floor((p.x - p.r - TS) / TS);
    const maxTx = Math.floor((p.x + p.r + TS) / TS);
    const minTy = Math.floor((p.y - p.r - TS) / TS);
    const maxTy = Math.floor((p.y + p.r + TS) / TS);
    const speed = Math.hypot(p.vx, p.vy);
    for (let ty = minTy; ty <= maxTy; ty++) {
      for (let tx = minTx; tx <= maxTx; tx++) {
        const t = world.get(tx, ty);
        if (t === EMPTY) continue;
        const rx = tx * TS;
        const ry = ty * TS;
        const nx = Math.max(rx, Math.min(p.x, rx + TS));
        const ny = Math.max(ry, Math.min(p.y, ry + TS));
        const dx = p.x - nx;
        const dy = p.y - ny;
        const d2 = dx * dx + dy * dy;
        if (d2 > p.r * p.r) continue;

        if (t === LAVA) {
          if (burnCd <= 0) {
            const dmg = Math.min(maxHp * 0.18, 14 * lavaMul * armorMul + maxHp * 0.03);
            p.hp -= dmg;
            burnCd = 0.85;
            hurtFlash = 1;
            shake = 12;
            popup(p.x, p.y - 16, `-${Math.round(dmg)} HP`, "#ff6b6b");
            burst(p.x, p.y, 20, "#ff9a3c", 220);
            const kx = p.vx === 0 ? 0 : -Math.sign(p.vx);
            p.vx = kx * 260 + (Math.random() - 0.5) * 60;
            p.vy = -320;
            p.fuel = Math.max(0, p.fuel - 6 * lavaMul);
          }
          continue;
        }

        const destroyed = speed > 45 || tileHardness(t, planet) === 0 ? hitTile(tx, ty, t) : false;
        if (destroyed) {
          const damp = tileHardness(t, planet) === 0 ? 0.995 : 0.965;
          p.vx *= damp;
          p.vy *= damp;
          continue;
        }
        const dist = Math.sqrt(d2) || 0.001;
        let nxn = dx / dist;
        let nyn = dy / dist;
        if (d2 < 0.0001) {
          nxn = 0;
          nyn = -1;
        }
        const push = p.r - dist;
        p.x += nxn * push;
        p.y += nyn * push;
        const dot = p.vx * nxn + p.vy * nyn;
        p.vx = (p.vx - 2 * dot * nxn) * 0.42;
        p.vy = (p.vy - 2 * dot * nyn) * 0.42;
        if (speed > 300) {
          shake = Math.max(shake, 4);
          burst(p.x, p.y, 5, "#cfd4dc", 90);
        }
      }
    }
    void dt;
  }

  function magnet() {
    if (magnetR <= 0) return;
    const rad = Math.ceil(magnetR / TS) + 1;
    const ptx = Math.floor(p.x / TS);
    const pty = Math.floor(p.y / TS);
    for (let ty = pty - rad; ty <= pty + rad; ty++) {
      for (let tx = ptx - rad; tx <= ptx + rad; tx++) {
        const t = world.get(tx, ty);
        if (t < ORE_BASE) continue;
        if (tileHardness(t, planet) > drill) continue;
        const dx = tx * TS + TS / 2 - p.x;
        const dy = ty * TS + TS / 2 - p.y;
        if (dx * dx + dy * dy <= magnetR * magnetR) hitTile(tx, ty, t);
      }
    }
  }

  /* --------------------------------- UFOs --------------------------------- */
  function spawnUfo() {
    const side = Math.random() > 0.5 ? 1 : -1;
    const x = p.x + side * (W / 2 + 80);
    const y = p.y - 120 - Math.random() * 80;
    ufos.push({ x, y, vx: -side * (60 + Math.random() * 40), hp: 3, shootCd: 1.5 + Math.random(), hover: Math.random() * Math.PI * 2 });
    popup(p.x, p.y - 30, "⚠ UFO INCOMING!", "#ff6b6b");
  }

  function updateUfos(dt: number) {
    if (!planet.ufos) return;
    const depth = Math.floor(p.y / TS);
    if (depth >= planet.ufoMinDepth) {
      ufoSpawnCd -= dt;
      if (ufoSpawnCd <= 0 && ufos.length < 3) {
        spawnUfo();
        ufoSpawnCd = 10 + Math.random() * 8;
      }
    }

    for (let i = ufos.length - 1; i >= 0; i--) {
      const u = ufos[i]!;
      u.hover += dt * 2;
      // Track the player horizontally, hover above
      const targetX = p.x;
      const dx = targetX - u.x;
      u.vx += dx * 0.5 * dt;
      u.vx *= 0.96;
      u.x += u.vx * dt;
      u.y = p.y - 130 + Math.sin(u.hover) * 20;

      u.shootCd -= dt;
      if (u.shootCd <= 0) {
        u.shootCd = 2.5 + Math.random() * 2;
        const sdx = p.x - u.x;
        const sdy = p.y - u.y;
        const slen = Math.hypot(sdx, sdy) || 1;
        projectiles.push({
          x: u.x,
          y: u.y + 10,
          vx: (sdx / slen) * 320,
          vy: (sdy / slen) * 320,
          life: 4,
          fromUfo: true,
        });
        burst(u.x, u.y + 10, 4, "#ff6b6b", 80);
      }
    }

    // Update projectiles
    for (let i = projectiles.length - 1; i >= 0; i--) {
      const pr = projectiles[i]!;
      pr.x += pr.vx * dt;
      pr.y += pr.vy * dt;
      pr.vy += GRAV * 0.15 * dt;
      pr.life -= dt;
      const pdx = pr.x - p.x;
      const pdy = pr.y - p.y;
      if (pdx * pdx + pdy * pdy < (p.r + 6) * (p.r + 6)) {
        const dmg = Math.min(maxHp * 0.12, 10 * armorMul + maxHp * 0.02);
        p.hp -= dmg;
        hurtFlash = 1;
        shake = 10;
        popup(p.x, p.y - 16, `-${Math.round(dmg)} HP`, "#ff6b6b");
        burst(p.x, p.y, 16, "#ff6b6b", 200);
        projectiles.splice(i, 1);
        continue;
      }
      if (pr.life <= 0) {
        projectiles.splice(i, 1);
      }
    }
  }

  function drawUfo(u: Ufo, ox: number, oy: number) {
    const x = u.x - ox;
    const y = u.y - oy;
    // Body
    ctx.fillStyle = "rgba(0,0,0,0.35)";
    ctx.beginPath();
    ctx.ellipse(x + 1, y + 8, 18, 5, 0, 0, Math.PI * 2);
    ctx.fill();
    // Saucer
    ctx.fillStyle = "#4a4a5a";
    ctx.beginPath();
    ctx.ellipse(x, y + 4, 20, 8, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#6a6a7a";
    ctx.beginPath();
    ctx.ellipse(x, y + 2, 16, 6, 0, 0, Math.PI * 2);
    ctx.fill();
    // Dome
    ctx.fillStyle = "#3ec4a0";
    ctx.beginPath();
    ctx.arc(x, y - 2, 9, Math.PI, 0);
    ctx.fill();
    ctx.fillStyle = "rgba(255,255,255,0.4)";
    ctx.fillRect(x - 5, y - 7, 4, 3);
    // Lights
    for (let i = -2; i <= 2; i++) {
      ctx.fillStyle = Math.sin(t * 4 + i) > 0 ? "#ff6b6b" : "#ffd166";
      ctx.fillRect(x + i * 6 - 1, y + 8, 2, 2);
    }
  }

  function drawProjectile(pr: Projectile, ox: number, oy: number) {
    const x = pr.x - ox;
    const y = pr.y - oy;
    ctx.fillStyle = "rgba(255,80,80,0.5)";
    ctx.beginPath();
    ctx.arc(x, y, 7, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#ff4040";
    ctx.beginPath();
    ctx.arc(x, y, 4, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#fff";
    ctx.fillRect(x - 1, y - 1, 2, 2);
  }

  /* --------------------------------- update -------------------------------- */
  function update(dt: number) {
    t += dt;
    if (state === "aim") {
      camX += (p.x - camX) * 0.1;
      camY += (0 - camY) * 0.1;
      return;
    }
    if (state === "over") return;

    const hasFuel = p.fuel > 0;
    const left = hasFuel && (keys.has("arrowleft") || keys.has("a") || touchDir === -1);
    const right = hasFuel && (keys.has("arrowright") || keys.has("d") || touchDir === 1);
    const steer = 1250 * speedMul;
    if (left) p.vx -= steer * dt;
    if (right) p.vx += steer * dt;

    if (!hasFuel && !wasDry) {
      wasDry = true;
      dryFlash = 2.6;
      popup(p.x, p.y - 20, "OUT OF FUEL — NO STEERING", "#ff6b6b");
    }
    if (hasFuel) wasDry = false;

    p.vy += GRAV * dt;
    const dragBase = hasFuel ? 0.16 : 1.5;
    p.vx -= p.vx * dragBase * dt;
    p.vy -= p.vy * dragBase * 0.35 * dt;

    const cap = 1000 * speedMul;
    p.vx = Math.max(-cap, Math.min(cap, p.vx));
    p.vy = Math.max(-cap, Math.min(cap * 1.2, p.vy));

    // Fuel burns ONLY from steering, at a slower base rate.
    const steering = left || right;
    if (steering) {
      p.fuel -= dt * 1.4 * FUEL_MUL;
    }
    p.fuel = Math.max(0, p.fuel);

    const speed = Math.hypot(p.vx, p.vy);
    const steps = Math.max(1, Math.ceil((speed * dt) / (TS * 0.4)));
    for (let i = 0; i < steps; i++) {
      p.x += (p.vx * dt) / steps;
      p.y += (p.vy * dt) / steps;
      collide(dt / steps);
    }
    magnet();

    updateUfos(dt);

    p.rot += (p.vx * dt) / p.r;

    maxDepth = Math.max(maxDepth, Math.floor(p.y / TS));
    burnCd -= dt;
    hurtFlash = Math.max(0, hurtFlash - dt * 2);
    shake *= 0.88;

    dryFlash = Math.max(0, dryFlash - dt);

    const drift = Math.hypot(p.x - anchorX, p.y - anchorY);
    if (drift > STOP_DRIFT) {
      anchorX = p.x;
      anchorY = p.y;
      stopTime = 0;
    } else {
      stopTime += dt;
      if (stopTime > STOP_TIME) {
        p.vx = 0;
        p.vy = 0;
        end(p.fuel <= 0 ? "Out of steam!" : "Rolled to a stop!");
      }
    }

    if (p.hp <= 0) {
      p.hp = 0;
      end("Shell cracked!");
    }

    camX += (p.x - camX) * 0.12;
    camY += (p.y + 60 - camY) * 0.1;

    for (let i = particles.length - 1; i >= 0; i--) {
      const q = particles[i]!;
      q.vy += GRAV * 0.4 * dt;
      q.x += q.vx * dt;
      q.y += q.vy * dt;
      q.life -= dt;
      if (q.life <= 0) particles.splice(i, 1);
    }
    for (let i = popups.length - 1; i >= 0; i--) {
      const u = popups[i]!;
      u.y -= dt * 28;
      u.life -= dt;
      if (u.life <= 0) popups.splice(i, 1);
    }
  }

  let endMsg = "";
  function end(msg: string) {
    if (state === "over") return;
    state = "over";
    endMsg = msg;
    burst(p.x, p.y, 40, "#ffd166", 260);
    setTimeout(() => {
      if (running)
        opts.onEnd({ depth: maxDepth, value, ores: collected, planet: planet.id });
    }, 900);
  }

  /* ---------------------------------- draw --------------------------------- */
  function drawSky(topY: number) {
    const g = ctx.createLinearGradient(0, 0, 0, Math.max(1, topY));
    g.addColorStop(0, planet.palette.sky[0]);
    g.addColorStop(1, planet.palette.sky[1]);
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, W, topY);
    if (planet.id === "moon") {
      // Stars on the moon
      ctx.fillStyle = "rgba(255,255,255,0.8)";
      for (let i = 0; i < 40; i++) {
        const sx = (i * 137 + t * 2) % W;
        const sy = (i * 53) % Math.max(1, topY);
        if (sy > topY) continue;
        ctx.fillRect(sx, sy, 2, 2);
      }
    } else if (planet.id === "mars") {
      // Dust haze
      ctx.fillStyle = "rgba(200,80,40,0.2)";
      ctx.fillRect(0, topY * 0.6, W, topY * 0.4);
    } else {
      // Clouds on Earth
      ctx.fillStyle = "rgba(255,255,255,0.85)";
      for (let i = 0; i < 6; i++) {
        const cx = ((i * 337 + t * 6) % (W + 200)) - 100;
        const cy = 30 + ((i * 53) % 70);
        if (cy > topY) continue;
        ctx.fillRect(cx, cy, 46, 10);
        ctx.fillRect(cx + 10, cy - 8, 26, 10);
      }
    }
  }

  function drawArmadillo(x: number, y: number, rot: number) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(rot);
    const r = p.r + 2;
    ctx.fillStyle = "rgba(0,0,0,0.35)";
    ctx.beginPath();
    ctx.arc(1.5, 2, r, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#9d6b45";
    ctx.beginPath();
    ctx.arc(0, 0, r, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#c28d5f";
    ctx.beginPath();
    ctx.arc(-r * 0.25, -r * 0.3, r * 0.72, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = "#6f4527";
    ctx.lineWidth = 2;
    for (let i = -2; i <= 2; i++) {
      ctx.beginPath();
      ctx.moveTo(i * (r / 2.6), -r);
      ctx.lineTo(i * (r / 2.6), r);
      ctx.stroke();
    }
    ctx.strokeStyle = "#4c2e18";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(0, 0, r - 0.5, 0, Math.PI * 2);
    ctx.stroke();
    ctx.fillStyle = "#f2d3b3";
    ctx.beginPath();
    ctx.arc(r * 0.55, r * 0.1, r * 0.34, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#2b1a0f";
    ctx.fillRect(r * 0.62, r * 0.02, 3, 3);
    ctx.restore();
  }

  function bandOf(ty: number) {
    return Math.max(0, Math.min(planet.palette.bands.length - 1, Math.floor(ty / 110)));
  }

  function draw() {
    const sx = (Math.random() - 0.5) * shake;
    const sy = (Math.random() - 0.5) * shake;
    const ox = Math.round(camX - W / 2 + sx);
    const oy = Math.round(camY - H * (state === "aim" ? 0.32 : 0.42) + sy);

    ctx.clearRect(0, 0, W, H);
    const surfaceY = -oy;
    if (surfaceY > 0) drawSky(Math.min(H, surfaceY));
    else {
      ctx.fillStyle = "#0a0a12";
      ctx.fillRect(0, 0, W, H);
    }

    if (surfaceY < H) {
      const band = planet.palette.bands[Math.min(bandOf(Math.floor((oy + H / 2) / TS)), planet.palette.bands.length - 1)]!;
      ctx.fillStyle = band.dark;
      const top = Math.max(0, surfaceY);
      ctx.fillRect(0, top, W, H - top);
      ctx.fillStyle = "rgba(0,0,0,0.28)";
      ctx.fillRect(0, top, W, H - top);
    }

    const tx0 = Math.floor(ox / TS) - 1;
    const tx1 = Math.floor((ox + W) / TS) + 1;
    const ty0 = Math.floor(oy / TS) - 1;
    const ty1 = Math.floor((oy + H) / TS) + 1;

    for (let ty = Math.max(0, ty0); ty <= ty1; ty++) {
      const band = bandOf(ty);
      for (let tx = tx0; tx <= tx1; tx++) {
        const tl = world.get(tx, ty)!;
        if (tl === EMPTY) continue;
        const dx = tx * TS - ox;
        const dy = ty * TS - oy;
        const v = Math.floor(hash2(tx, ty, 5) * 8);
        if (tl === LAVA) {
          const glow = 0.6 + 0.4 * Math.sin(t * 3 + tx + ty);
          ctx.fillStyle = "#7a1e05";
          ctx.fillRect(dx, dy, TS, TS);
          ctx.fillStyle = `rgba(255,${90 + glow * 70},20,1)`;
          ctx.fillRect(dx + 1, dy + 1, TS - 2, TS - 2);
          ctx.fillStyle = `rgba(255,230,140,${0.35 + glow * 0.4})`;
          ctx.fillRect(dx + 3, dy + 3 + Math.sin(t * 4 + tx) * 2, TS - 8, 3);
          continue;
        }
        if (tl === BEDROCK) {
          ctx.fillStyle = "#15100c";
          ctx.fillRect(dx, dy, TS, TS);
          ctx.fillStyle = "#241a13";
          ctx.fillRect(dx + 2, dy + 2, TS - 4, TS - 4);
          continue;
        }
        const px = (((tx % PATCH) + PATCH) % PATCH) * TS;
        const py = (((ty % PATCH) + PATCH) % PATCH) * TS;
        if (tl === ROCK) {
          ctx.drawImage(rockPatch(band, planet), px, py, TS, TS, dx, dy, TS, TS);
        } else {
          ctx.drawImage(dirtPatch(band, planet), px, py, TS, TS, dx, dy, TS, TS);
          if (tl === GRASS) ctx.drawImage(grassSprite(v, planet), dx, dy);
          if (tl >= ORE_BASE) ctx.drawImage(oreSprite(tl - ORE_BASE, v, planet), dx, dy);
        }
        if (world.get(tx, ty - 1) === EMPTY) {
          ctx.fillStyle = "rgba(255,236,200,0.18)";
          ctx.fillRect(dx, dy, TS, 2);
        }
        if (world.get(tx, ty + 1) === EMPTY) {
          ctx.fillStyle = "rgba(0,0,0,0.4)";
          ctx.fillRect(dx, dy + TS - 3, TS, 3);
        }
        if (world.get(tx - 1, ty) === EMPTY) {
          ctx.fillStyle = "rgba(0,0,0,0.25)";
          ctx.fillRect(dx, dy, 2, TS);
        }
        if (world.get(tx + 1, ty) === EMPTY) {
          ctx.fillStyle = "rgba(0,0,0,0.25)";
          ctx.fillRect(dx + TS - 2, dy, 2, TS);
        }
      }
    }

    const dark = Math.min(0.45, Math.max(0, (camY / TS - 150) / 4000));
    if (dark > 0) {
      ctx.fillStyle = `rgba(0,0,0,${dark})`;
      ctx.fillRect(0, 0, W, H);
    }

    for (const q of particles) {
      ctx.globalAlpha = Math.max(0, q.life / q.max);
      ctx.fillStyle = q.c;
      ctx.fillRect(q.x - ox, q.y - oy, q.s, q.s);
    }
    ctx.globalAlpha = 1;

    // UFOs and projectiles
    for (const u of ufos) drawUfo(u, ox, oy);
    for (const pr of projectiles) drawProjectile(pr, ox, oy);

    drawArmadillo(p.x - ox, p.y - oy, p.rot);

    if (state === "aim") {
      const cx = p.x - ox;
      const cy = p.y - oy;
      if (aiming) {
        let dx = cx - aimX;
        let dy = cy - aimY;
        const len = Math.hypot(dx, dy) || 1;
        const power = Math.min(len, 170) / 170;
        dx /= len;
        dy /= len;
        const speed = (420 + power * 780) * powerMul;
        const vx = dx * speed;
        let vy = Math.max(dy * speed, -220);
        let x = cx;
        let y = cy;
        for (let i = 0; i < 34; i++) {
          x += vx * 0.035;
          y += vy * 0.035;
          vy += GRAV * 0.035;
          ctx.globalAlpha = 1 - i / 36;
          ctx.fillStyle = "#fff2b8";
          ctx.fillRect(x - 2, y - 2, 4, 4);
        }
        ctx.globalAlpha = 1;
        ctx.fillStyle = "rgba(0,0,0,0.55)";
        ctx.fillRect(cx - 42, cy - 46, 84, 14);
        ctx.fillStyle = power > 0.8 ? "#ff7a4d" : "#ffd166";
        ctx.fillRect(cx - 40, cy - 44, 80 * power, 10);
      }
      ctx.fillStyle = "rgba(0,0,0,0.55)";
      ctx.fillRect(W / 2 - 190, H - 96, 380, 56);
      ctx.fillStyle = "#fff8e6";
      ctx.font = "bold 20px 'Baloo 2', system-ui, sans-serif";
      ctx.textAlign = "center";
      ctx.fillText("Drag back and release to launch!", W / 2, H - 68);
      ctx.font = "16px 'Baloo 2', system-ui, sans-serif";
      ctx.fillStyle = "#ffe9a8";
      ctx.fillText("Steer with  A / D  or  ← →  — needs fuel!", W / 2, H - 48);
    }

    for (const q of popups) {
      ctx.globalAlpha = Math.min(1, q.life);
      ctx.font = "bold 17px 'Baloo 2', system-ui, sans-serif";
      ctx.textAlign = "center";
      ctx.lineWidth = 4;
      ctx.strokeStyle = "rgba(0,0,0,0.8)";
      ctx.strokeText(q.text, q.x - ox, q.y - oy);
      ctx.fillStyle = q.c;
      ctx.fillText(q.text, q.x - ox, q.y - oy);
    }
    ctx.globalAlpha = 1;

    if (hurtFlash > 0) {
      ctx.fillStyle = `rgba(255,40,40,${hurtFlash * 0.35})`;
      ctx.fillRect(0, 0, W, H);
    }

    drawHud();

    if (state === "over") {
      ctx.fillStyle = "rgba(0,0,0,0.55)";
      ctx.fillRect(0, 0, W, H);
      ctx.textAlign = "center";
      ctx.fillStyle = "#ffd166";
      ctx.font = "bold 42px 'Baloo 2', system-ui, sans-serif";
      ctx.fillText(endMsg, W / 2, H / 2);
    }
  }

  function bar(
    x: number,
    y: number,
    w: number,
    h: number,
    pct: number,
    fill: string,
    label: string,
    icon: string,
  ) {
    ctx.fillStyle = "rgba(0,0,0,0.6)";
    ctx.fillRect(x - 4, y - 4, w + 8, h + 8);
    ctx.fillStyle = "#2a2018";
    ctx.fillRect(x, y, w, h);
    ctx.fillStyle = fill;
    ctx.fillRect(x, y, w * Math.max(0, Math.min(1, pct)), h);
    ctx.fillStyle = "rgba(255,255,255,0.28)";
    ctx.fillRect(x, y, w * Math.max(0, Math.min(1, pct)), h * 0.35);
    ctx.strokeStyle = "rgba(255,255,255,0.85)";
    ctx.lineWidth = 2;
    ctx.strokeRect(x - 0.5, y - 0.5, w + 1, h + 1);
    ctx.textAlign = "left";
    ctx.font = "bold 16px 'Baloo 2', system-ui, sans-serif";
    ctx.lineWidth = 4;
    ctx.strokeStyle = "rgba(0,0,0,0.85)";
    ctx.strokeText(`${icon} ${label}`, x + 6, y + h - 5);
    ctx.fillStyle = "#ffffff";
    ctx.fillText(`${icon} ${label}`, x + 6, y + h - 5);
  }

  function drawHud() {
    bar(
      16,
      16,
      Math.min(300, W * 0.42),
      26,
      p.hp / maxHp,
      "#e63946",
      `${Math.ceil(p.hp)} / ${Math.round(maxHp)}`,
      "❤",
    );
    bar(
      16,
      54,
      Math.min(220, W * 0.32),
      16,
      p.fuel / fuelMax,
      "#3ec46d",
      `${Math.ceil(p.fuel)} fuel`,
      "⚡",
    );
    ctx.textAlign = "right";
    ctx.font = "bold 26px 'Baloo 2', system-ui, sans-serif";
    ctx.lineWidth = 5;
    ctx.strokeStyle = "rgba(0,0,0,0.85)";
    const depthText = `⬇ ${maxDepth} m`;
    const coinText = `🪙 ${value}`;
    ctx.strokeText(depthText, W - 16, 40);
    ctx.fillStyle = "#ffe9a8";
    ctx.fillText(depthText, W - 16, 40);
    ctx.strokeText(coinText, W - 16, 70);
    ctx.fillStyle = "#ffd166";
    ctx.fillText(coinText, W - 16, 70);

    // Planet label
    ctx.textAlign = "left";
    ctx.font = "bold 14px 'Baloo 2', system-ui, sans-serif";
    ctx.lineWidth = 3;
    ctx.strokeStyle = "rgba(0,0,0,0.85)";
    const planetText = `${planet.icon} ${planet.name}`;
    ctx.strokeText(planetText, 16, 78);
    ctx.fillStyle = "#c4a878";
    ctx.fillText(planetText, 16, 78);

    if (state !== "roll") return;

    if (p.fuel <= 0) {
      const pulse = 0.55 + 0.45 * Math.sin(t * 7);
      ctx.textAlign = "left";
      ctx.font = "bold 15px 'Baloo 2', system-ui, sans-serif";
      ctx.globalAlpha = dryFlash > 0 ? 1 : 0.55 + pulse * 0.45;
      ctx.lineWidth = 4;
      ctx.strokeStyle = "rgba(0,0,0,0.85)";
      const msg = "⚠ NO FUEL — STEERING LOCKED";
      ctx.strokeText(msg, 16, 98);
      ctx.fillStyle = "#ff9a9a";
      ctx.fillText(msg, 16, 98);
      ctx.globalAlpha = 1;
    }

    if (stopTime > 0.22) {
      const pct = Math.min(1, stopTime / STOP_TIME);
      const w = 190;
      const x = W / 2 - w / 2;
      const y = H - 62;
      ctx.fillStyle = "rgba(0,0,0,0.6)";
      ctx.fillRect(x - 4, y - 4, w + 8, 22);
      ctx.fillStyle = "#2a2018";
      ctx.fillRect(x, y, w, 14);
      ctx.fillStyle = pct > 0.65 ? "#e63946" : "#ffd166";
      ctx.fillRect(x, y, w * pct, 14);
      ctx.strokeStyle = "rgba(255,255,255,0.85)";
      ctx.lineWidth = 2;
      ctx.strokeRect(x - 0.5, y - 0.5, w + 1, 15);
      ctx.textAlign = "center";
      ctx.font = "bold 14px 'Baloo 2', system-ui, sans-serif";
      ctx.lineWidth = 4;
      ctx.strokeStyle = "rgba(0,0,0,0.85)";
      ctx.strokeText("STOPPING", W / 2, y + 12);
      ctx.fillStyle = "#ffffff";
      ctx.fillText("STOPPING", W / 2, y + 12);
    }
  }

  /* ---------------------------------- loop --------------------------------- */
  let last = performance.now();
  function frame(now: number) {
    if (!running) return;
    const dt = Math.min(0.033, (now - last) / 1000);
    last = now;
    update(dt);
    draw();
    requestAnimationFrame(frame);
  }
  requestAnimationFrame(frame);

  return {
    destroy() {
      running = false;
      ro.disconnect();
      canvas.removeEventListener("pointerdown", onDown);
      canvas.removeEventListener("pointermove", onMove);
      window.removeEventListener("pointerup", onUp);
      window.removeEventListener("keydown", onKeyDown);
      window.removeEventListener("keyup", onKeyUp);
    },
  };
}
